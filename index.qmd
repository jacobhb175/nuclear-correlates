...
plot1<-fit_nuclear |>
  ...
  ggplot(aes(x = .epred, fill = as.factor(great))) +
  stat_slab(alpha=0.8) +
  scale_fill_calc() +
  labs(fill = "Great") +
...
plot2<-fit_nuclear |>
  add_epred_draws(newdata = ndata) |>
  ggplot(aes(x = .epred, fill = as.factor(nuclear))) +
  stat_slab(alpha=0.8) +
  scale_fill_calc() +
  labs(fill = "Nuclear")+
  scale_x_continuous(labels=scales::percent_format())
...
plot3<-fit_nuclear |>
  ...
  ggplot(aes(x = .epred, fill = as.factor(value))) +
  stat_slab(alpha=0.8) +
  scale_fill_calc() +
  labs(fill = "Great/Nuclear Powers") +
  scale_x_continuous(labels=scales::percent_format()) +
...
plot4<-fit_nuclear |>
  add_epred_draws(newdata = ndata) |>
  pivot_longer(cols = c(great, nuclear), names_to = "category", values_to = "value") |>
  ggplot(aes(x = .epred, fill = interaction(category, as.factor(value)))) +
  stat_slab(alpha=0.8) +
  scale_fill_calc() +
  labs(fill = "Category") +
  scale_x_continuous(labels=scales::percent_format())
...
plot4 <- combined_data |>
  ggplot(aes(x = .epred, y = category, fill = as.factor(value))) +
  stat_slab(alpha=0.8) +
  scale_fill_calc() +
  scale_x_continuous(labels=scales::percent_format()) +
  scale_y_discrete(labels=c("Great","Nuclear")) +
  labs(y="Category",x="Predicted Chance of High Escalation",fill="Involved Powers")
