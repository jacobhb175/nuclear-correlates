...
fit_nuclear<-brm(formula = hihost ~ modern + great + nuclear, data = nuclear_data, family = gaussian(), refresh = 0, silent = 2, seed = 19)
```
...
```{r}
#| label: plot2
ndata<-tibble::tibble(expand_grid(modern=c(TRUE,FALSE),great=c(1,2),nuclear=c(1,2)))

fit_nuclear |>
add_epred_draws(newdata=ndata) |> 
  ggplot(aes(x=.epred,fill=modern,y=great))+stat_slab()+scale_fill_calc()
```



fit_nuclear |> add_epred_draws(newdata=ndata) |> 
  ggplot(aes(x=.epred,fill=as.factor(great)))+geom_histogram(bins=100)
